...
public class UserRepository {
    ...
    public CompletableFuture<Integer> getActivityParticipantCount(Long activityId) {
        CompletableFuture<Integer> future = new CompletableFuture<>();
        apiService.getActivityParticipantCount(activityId).enqueue(new Callback<ApiResponse<Integer>>() {
            @Override
            public void onResponse(Call<ApiResponse<Integer>> call, Response<ApiResponse<Integer>> response) {
                if (response.isSuccessful() && response.body() != null && response.body().isSuccess()) {
                    // 直接获取Integer类型的data（服务器返回的数字）
                    Integer count = response.body().getData();
                    future.complete(count != null ? count : 0);
                } else {
                    future.complete(0);
                }
            }
    ...
    public CompletableFuture<List<HashMap<String, Object>>> getActivityParticipants(Long activityId) {
        ...
        apiService.getActivityParticipants(activityId).enqueue(new Callback<ApiResponse<List<HashMap<String, Object>>>>() {
            @Override
            public void onResponse(Call<ApiResponse<List<HashMap<String, Object>>>> call, Response<ApiResponse<List<HashMap<String, Object>>>> response) {
                if (response.isSuccessful() && response.body() != null && response.body().isSuccess()) {
                    // 直接获取List类型的data（服务器返回的参与者列表）
                    List<HashMap<String, Object>> participants = response.body().getData();
                    future.complete(participants != null ? participants : new ArrayList<>());
                } else {
            ...
            @Override
            public void onFailure(Call<ApiResponse<List<HashMap<String, Object>>>> call, Throwable t) {
                future.complete(new ArrayList<>());
            }
        });
        return future;
